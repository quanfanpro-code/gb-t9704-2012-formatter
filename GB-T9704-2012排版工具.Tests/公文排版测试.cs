...
{
    // 排版测试只固定外部字体清单，仍运行真实文档处理；缺字体路径在独立用例中检查。
    [Theory]
    ...
    [InlineData(排版模式.电子红头)]
    [InlineData(排版模式.套打)]
    public void 正式模式缺失字体必须阻断且保留原文(排版模式 模式)
    {
        var input = 样本("关于开展检查的通知", "正文。", "2026年9月12日");
        var original = File.ReadAllBytes(input);
        var output = Path.Combine(Path.GetDirectoryName(input)!, "正式成稿.docx");
        var result = new GovDocumentPipeline(() => ["仿宋_GB2312"]).Process(new RequestContract
        {
            InputPath = input,
            OutputPath = output,
            模式 = 模式,
        ...
        using var audit = JsonDocument.Parse(File.ReadAllText(result.AuditPath!));
        Assert.Equal("阻断", audit.RootElement.GetProperty("GateResult").GetString());
        Assert.Contains("仿宋_GB2312", audit.RootElement.GetProperty("实际检查记录").ToString());
    }
